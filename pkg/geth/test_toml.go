...
// mainnetConfigTOML generated from running `geth dumpconfig`
const mainnetConfigTOML = `[Eth]
NetworkId = 1
SyncMode = "snap"
...
DatasetDir = "/Users/z/Library/Ethash"
DatasetsInMem = 1
DatasetsOnDisk = 2
...
[Node]
DataDir = "/Users/z/Library/Ethereum"
IPCPath = "geth.ipc"
HTTPHost = ""
HTTPPort = 8545
...
// devConfigTOML generated from running `geth --dev dumpconfig`
const devConfigTOML = `[Eth]
NetworkId = 1337
SyncMode = "full"
EthDiscoveryURLs = []
SnapDiscoveryURLs = []
...
func Test_dumpDefaultMainnetConfig(t *testing.T) {
	is := assert.New(t)

	cfg := MainnetGethConfig()

	var b bytes.Buffer
	writer := bufio.NewWriter(&b)

	err := dumpConfig(cfg, writer)
	is.NoError(err)

	is.Equal(mainnetConfigTOML, b.String())
}

func Test_dumpDeveloperConfig(t *testing.T) {
	is := assert.New(t)
	...
	err = dumpConfig(cfg, os.Stdout)
	is.NoError(err)

	is.Equal(devConfigTOML, b.String())
}
